...
VOID ImageLoad(IMG img, VOID *v)
{
    ...
    RTN rtnrealloc = RTN_FindByName(img, "realloc");
    if ( RTN_Valid( rtnrealloc ))
    {
        RTN_Open(rtnrealloc);
        RTN_InsertCall(rtnrealloc, IPOINT_BEFORE, AFUNPTR(BeforeRealloc), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_THREAD_ID, IARG_END);
        RTN_Close(rtnrealloc);
    }
    ...
    if(RTN_Valid(rtnpthread_barrier_wait))
    {
        ...
        RTN_InsertCall(rtnpthread_barrier_wait, IPOINT_AFTER, AFUNPTR(AfterPthread_barrier_wait), IARG_THREAD_ID, IARG_END);
